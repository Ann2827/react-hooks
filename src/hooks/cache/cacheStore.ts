...
import { TCacheState, ICacheData } from './cache.types';
...
const CacheStore = makeStore<TCacheState>(initialState, dataOptions).enrich<ICacheData>(
  (setState, { state, reset, on }) => {
    // Private
    const checkPlace = (place: 'localStorage') => {
      ...
      setState((prev) => ({ ...prev, checks: { ...prev, [place]: check } }));
      return check;
    };
    ...
    const setCache: ICacheData['setCache'] = (data): ReturnType<ICacheData['setCache']> => {
      ...
      data.forEach((item) => {
        ...
        action(
          ...
          'set',
          `${prefix}-${key}`,
          JSON.stringify({ maxAge: expires ? new Date(Date.now() + 1000 * 60 * expires) : null, value }),
        );
      });
    };
    ...
    const getCache: ICacheData['getCache'] = (data): ReturnType<ICacheData['getCache']> => {
      ...
      (Object.keys(data) as Array<keyof typeof data>).forEach((key) => {
        ...
        try {
          ...
          if (!('maxAge' in objValue) || objValue.maxAge === null || new Date(objValue.maxAge).getTime() >= Date.now())
